...
-- Permisos de lectura para usuario1 sobre tablas específicas
GRANT SELECT ON sistemaQA.Usuario TO 'usuario1'@'localhost';
GRANT SELECT ON sistemaQA.Modulo TO 'usuario1'@'localhost';

-- Permisos de lectura y escritura para usuario2 sobre todas las tablas
GRANT SELECT, INSERT, UPDATE ON sistemaQA.* TO 'usuario2'@'localhost';

-- Permisos de lectura y eliminación para usuario3 sobre todas las tablas
GRANT SELECT, DELETE ON sistemaQA.* TO 'usuario3'@'localhost';

-- Probar permisos mediante operaciones DML
-- 1. Usuario 1 intenta insertar (debe fallar)
-- 2. Usuario 2 inserta un nuevo registro en la tabla Usuario
-- 3. Usuario 3 intenta eliminar un registro de la tabla Usuario

-- Iniciar sesión como usuario1 para verificar permisos
-- (Esto se realiza fuera de este script utilizando la conexión al servidor).

-- Transacciones con TCL
START TRANSACTION;

-- Insertar registro como usuario2
INSERT INTO Usuario (Nombre, Email, Rol) VALUES ('Test User 2', 'test2@example.com', 'QA Tester');

-- Confirmar operación
COMMIT;

-- Intentar eliminar un registro como usuario3
START TRANSACTION;
DELETE FROM Usuario WHERE Email = 'test2@example.com';

-- Deshacer operación (Rollback)
ROLLBACK;

-- Transferir permisos de usuario3 a usuario2
REVOKE SELECT, DELETE ON sistemaQA.* FROM 'usuario3'@'localhost';
GRANT SELECT, DELETE ON sistemaQA.* TO 'usuario2'@'localhost';
